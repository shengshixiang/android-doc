...
/*modified by yjc @20210708, for MC538 disable hs uart1 pm runtime at customer's request start*/
#define DISABLE_PM_RUNTIME
/*modified by yjc @20210708, for MC538 disable hs uart1 pm runtime at customer's request end*/
...
#ifdef DISABLE_PM_RUNTIME
	if(!uart_console(uport) && (uport->line == 1))
	{
		return 0;
	}
	else
...
#if defined(DISABLE_PM_RUNTIME)
	if (!pm_runtime_enabled(uport->dev) && (uport->line == 0)) {
...
/**
 * reference code from: drivers/tty/serial/serial_core.c -> static int uart_write(struct tty_struct *tty, const unsigned char *buf, int count)
 */
static void serial_spdata_write(unsigned char *buf,unsigned int count)
{
   struct uart_port *port = sp_port;
...
static int msm_geni_serial_startup(struct uart_port *uport)
{
...
#ifdef DISABLE_PM_RUNTIME
	/*if (likely(!uart_console(uport)) && (uport->line == 1)) {
		ret = meige_msm_geni_serial_power_on(msm_port);
		if (ret) {
	...
	if (likely(!uart_console(uport)) && (uport->line == 0)) {
		ret = msm_geni_serial_power_on(&msm_port->uport);
		if (ret) {
			dev_err(uport->dev, "%s:Failed to power on %d\n",
							__func__, ret);
...
static int msm_geni_serial_probe(struct platform_device *pdev)
{
	...
	/* [FEATURE]-Add-BEGIN by luozhenhui@paxsz.com 2022-09-16, for sp PedGetPinBlock */
	   ...
	   if (!is_console && uport->line == 1){
		   ...
		   sp_port = uport;
		   spdata_func = kzalloc(sizeof(struct tp_inject_func), GFP_KERNEL);
		   if (NULL != spdata_func) {
			   spdata_func->write = (void *)serial_spdata_write;
			   SpToAp_FuncRegister(spdata_func);
		   }
...
#ifdef DISABLE_PM_RUNTIME
	if (likely(!uart_console(uport)) && (uport->line == 1)) {
		ret = meige_msm_geni_serial_power_on(dev_port);
		if (ret) {
			dev_err(uport->dev, "%s:Failed to power on ttyHS1 %d\n",
							__func__, ret);
			//return ret;
		}
	}
...
#ifdef DISABLE_PM_RUNTIME
bool hs_uart_poweron_staus = false;
static int meige_msm_geni_serial_power_on(struct msm_geni_serial_port *port)
{
	int ret = 0;
	if(hs_uart_poweron_staus == true)
	{		
		pr_err("yjc meige_msm_geni_serial_power_on already power on\n");
		return 0;
	}
	pr_err("yjc meige_msm_geni_serial_power_on start\n");
	...
	/* Enable interrupt */
	enable_irq(port->uport.irq);
	hs_uart_poweron_staus = true;

	IPC_LOG_MSG(port->ipc_log_pwr, "%s:\n", __func__);	
	pr_err("yjc meige_msm_geni_serial_power_on end\n");
exit_runtime_resume:
	return ret;
}
#endif
...
static int msm_geni_serial_sys_suspend_noirq(struct device *dev)
{
	...
	#ifdef DISABLE_PM_RUNTIME
		CDBG("yjc msm_geni_serial_sys_suspend_noirq  DISABLE_PM_RUNTIME\n");
		if(uport->line == 1)
		{
...
static int msm_geni_serial_sys_resume_noirq(struct device *dev)
{
	...
	if (!uart_console(uport) && uport->suspended && (uport->line == 1)) {

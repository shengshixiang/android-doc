...
def load_config_file(filename, default_filename=None,
    ...
    if not os.path.exists(filename):
        config['unreleased_version_label'] = "unreleased"
        config['ignore_regexps'] = []
        config['section_regexps'] = [(None, '')]
        config['tag_filter_regexp'] = r"\d+\.\d+(\.\d+)?"
...
REGEX_RFC822_KEY_VALUE_EX = \
    r'(^|\n)(\[)(?P<key>[A-Z]\w+([ -]\w+)*)(\]):( |\n)*(?P<value>[^\n]*(\n[^\n]*)*)'

REGEX_RFC822_POSTFIX = \
    r'(%s)+$' % REGEX_RFC822_KEY_VALUE_EX
...
class GitRepos(object):
    ...
    def get_branch(self):
        ...
        os.chdir(self.toplevel)
        try:
            branch = wrap("git symbolic-ref --short -q HEAD")
        except:
            branch = os.environ.get("GIT_BRANCH")
            if branch != None:
                branch = branch + "\n";
            else:
                branch=self.commit("HEAD").sha1_short + "\n"
    ...
    def get_remote(self):
        ...
        os.chdir(self.toplevel)
        try:
            remote_str = wrap("git remote get-url origin")
        except:
            remote_str = "unknown\n"
        os.chdir(cwd)
        return remote_str
    ...
    def get_ver(self):
        ...
        os.chdir(self.toplevel)
        try:
            ver_str = wrap("git describe --tags")
            index = ver_str.find("\n")
            ver_str = ver_str[0:index+1]
        except:
            ver_str = "unknown\n"
        print(ver_str)
        os.chdir(cwd)
    ...
    def get_head_commit(self):
        cwd = os.getcwd()
        os.chdir(self.toplevel)
        commit_str=self.commit("HEAD").sha1_short + "\n"
        os.chdir(cwd)
        return commit_str
...
def excel_py(data, opts={}):
    ...
    try:
        wb = load_workbook(filename = data["excel_path"])
    except:
        print("load error")
        wb = Workbook()
        ws = wb.active
    ...
    try:
        ws = wb.get_sheet_by_name(data["ver"])
    except Exception as e:
        ws = wb.create_sheet(data["ver"],0)
    ...
    ws['A1'] = data["product"] + " "+data["ver"]+" ChangeLog"
    ws['A2'] = data["paxversion"]
    ws['A3'] = data["msg"]

    model_list = data["model"].split(',')
    model_show = "NA"

    if len(model_list) > 1:
        data["model"] = "ALL,"+data["model"];
    else:
        model_show = data["model"];
        data["model"] = "ALL,"+data["model"];

    dv = DataValidation(type="list", formula1='"TODO,READY,NULL"', allow_blank=True)
    dvm = DataValidation(type="list", formula1='"{}"'.format(data["model"]), allow_blank=True)

    dvm.error=dv.error ='Your entry is not in the list'
    dvm.errorTitle=dv.errorTitle = 'Invalid Entry'
    dvm.prompt=dv.prompt = 'Please select from the list'
    dvm.promptTitle=dv.promptTitle = 'List Selection'

    # ws.add_data_validation(dvm)
    # ws.add_data_validation(dv)

    msg_list = data["msg"].split('\n')
    ws.row_dimensions[3].height = 20*len(msg_list)
    ...
    font0 = Font(size=11)
    font1 = Font(size=20, b=True)
    font2 = Font(size=14, b=True)
    font3 = Font(size=11)
    font4 = Font(size=13)
    ...
    border0 = Border(top=line_t, bottom=line_t, left=line_t, right=line_t)
    border1 = Border(top=line_m, bottom=line_m, left=line_t, right=line_t)
    border2 = Border(top=line_m, bottom=line_m, left=line_t, right=line_t)
    border3 = Border(top=line_m, bottom=line_m, left=line_t, right=line_t)
    border4 = Border(top=line_m, bottom=line_m, left=line_t, right=line_t)

    # 填充
    fill1 = PatternFill('solid', fgColor='DCE6F1')
    fill2 = PatternFill('solid', fgColor='B8CCE4')
    fill3 = PatternFill('solid', fgColor='DCE6F1')
    fill4 = PatternFill('solid', fgColor='B8CCE4')

    alignment0=Alignment(horizontal='left',
        vertical='center',
        text_rotation=0,
        wrap_text=True,
        shrink_to_fit=False,
    ...
    alignment3=Alignment(horizontal='left',
        vertical='center',
        text_rotation=0,
        wrap_text=True,
        shrink_to_fit=False,
    ...
    alignment4=Alignment(horizontal='left',
        vertical='center',
        text_rotation=0,
        wrap_text=True,
        shrink_to_fit=False,
    ...
    # 将样式打包命名
    sty0 = NamedStyle(name='sty0', font=font0,fill=fill1, alignment=alignment0, border=border0)
    sty1 = NamedStyle(name='sty1', font=font1,fill=fill1, alignment=alignment1, border=border1)
    sty2 = NamedStyle(name='sty2', font=font2,fill=fill2, alignment=alignment2, border=border2)
    sty3 = NamedStyle(name='sty3', font=font3,fill=fill3, alignment=alignment3, border=border3)
    sty4 = NamedStyle(name='sty4', font=font4,fill=fill4, alignment=alignment4, border=border4)

    if excel_new:
        try:
            wb.add_named_style(sty0)
        except:
            pass

        try:
            wb.add_named_style(sty1)
        except:
            pass

        try:
            wb.add_named_style(sty2)
        except:
            pass

        try:
            wb.add_named_style(sty3)
        except:
            pass

        try:
            wb.add_named_style(sty4)
        except:
            pass

    pax_ex = ["变更人","版本","状态","修改时间","类型","代码","影响机型、平台或产品形态","BUG","变更标题(Title)","详细说明：需求背景、目的、修改详情(Summary)","自测试步骤、影响范围及测试建议（Test Plan）","模块"]

    for i in range(len(pax_ex)):
        ws.cell(4, i+1).value = pax_ex[i]
    ...
    if sheet_new:
        ...
        for i in range(len(pax_ex)):
            try:
                ws.cell(4, i+1).style = 'sty4'
            except:
                pass

        for i in range(1,13):
            try:
                ws.cell(1, i).style = 'sty1'
                ws.cell(2, i).style = 'sty2'
                ws.cell(3, i).style = 'sty3'
            except:
                pass

    cell_cnt = 5

    datacomit = [data["versions"],data["versions2"]];

    for i in range(2):
        for version in datacomit[i]:
            if len(version["sections"]) > 0:
                sections = version["sections"]
                for section in sections:
                    for commit in section["commits"]:
                        ws.insert_rows(cell_cnt, 1)
                        subject = commit["subject"]
                        mycommit = commit["commit"]
                        # reviewers = mycommit.trailer_reviewers
                        # reviewed_by = mycommit.trailer_summary

                        ws.cell(cell_cnt, 1).value = commit["author"] 
                        # mytime=datetime.datetime.now()+datetime.timedelta(hours=8)
                        mytime=datetime.datetime.now()
                        ws.cell(cell_cnt, 2).value =  mytime.strftime("%Y%m%d")

                        try:
                            ws.cell(cell_cnt, 11).value = mycommit.trailer_test_plan
                            # print ("test plan " + mycommit.trailer_test_plan)
                        except:
                            ws.cell(cell_cnt, 11).value = ""

                        if ws.cell(cell_cnt, 11).value == "NO" or ws.cell(cell_cnt, 11).value == "NO\n":
                            ws.cell(cell_cnt, 3).value = "NULL"
                        else:
                            ws.cell(cell_cnt, 3).value = "TODO"

                        # dv.add(ws.cell(cell_cnt, 3))
                        ws.cell(cell_cnt, 4).value = mycommit.date
                        ws.cell(cell_cnt, 5).value = commit["type"]
                        ws.cell(cell_cnt, 6).value = mycommit.sha1_short
                        ws.cell(cell_cnt, 7).value = model_show
                        # dvm.add(ws.cell(cell_cnt, 7))
                        #try:
                         #   ws.cell(cell_cnt, 8).value = mycommit.trailer_bug
                        #except:
                        #    ws.cell(cell_cnt, 8).value = ""
                        ws.cell(cell_cnt, 9).value = subject[8:len(subject)]
                        tmp = subject
                        index = tmp.lower().find("bug")
                        print("index = ",index)
                        flag=0
                        a=0
                        if(index > 0):
                            for c in range(index+3, len(subject)):
                                try:
                                    unicodedata.numeric(subject[c])
                                    if flag == 0:
                                        a = c
                                        flag = 1
                                    ...
                                    if flag == 1: 
                                        break	
                                    pass
                            ws.cell(cell_cnt, 8).value = subject[a:c]
                        try:
                            ws.cell(cell_cnt, 10).value = mycommit.trailer_summary
                        except:
                            ws.cell(cell_cnt, 10).value = ""

                        if ws.cell(cell_cnt, 10).value == "" and ws.cell(cell_cnt, 11).value == "":
                            try:
                                ws.cell(cell_cnt, 10).value = mycommit.body
                            except:
                                ws.cell(cell_cnt, 10).value = ""

                        try:
                            ws.cell(cell_cnt, 12).value = mycommit.trailer_module
                        except:
                            ws.cell(cell_cnt, 12).value = ""
							
                        cell_cnt = cell_cnt + 1
                        #yield subject + "\n\n"

    # tab = Table(displayName="Table1", ref="A4:K1048576")
...
def versions_data_iter(commit_type=None, repository=None, revlist=None,
                       ignore_regexps=[],
...
def changelog(output_engine=rest_py,
    ...
    if kwargs.get("revlist"):
       print(kwargs.get("revlist"))
       str_test=kwargs.get("revlist")[0].split("..")
       if len(str_test) > 1:
           paxversion = str_test[1]
       else:
           paxversion = str_test[0]
    ...
    ap_repo = repository;
    versions = versions_data_iter(warn=warn, commit_type="AP", repository=ap_repo, **kwargs)
    data["msg"] = "AP_REPO: " + ap_repo.get_remote() + "AP_BRANCH: " + ap_repo.get_branch()+ "AP_COMMIT: " + ap_repo.get_head_commit() + "AP_VER: "+ ap_repo.get_ver() + "PosApi_VER:"

    ## poke once in versions to know if there's at least one:
    try:
        first_version = next(versions)
    except StopIteration:
        warn("AP Empty changelog. No commits were elected to be used as entry.")
        data["versions"] = []
    else:
        data["versions"] = itertools.chain([first_version], versions)

    if sp_dir:
        sp_repo = GitRepos(os.path.realpath(sp_dir));
        versions2 = versions_data_iter(warn=warn, commit_type="SP", repository=sp_repo, **kwargs)
        data["msg"] = data["msg"] + "\nSP_REPO: " + sp_repo.get_remote() + "SP_BRANCH: " + sp_repo.get_branch()+ "SP_COMMIT: " + sp_repo.get_head_commit()

        try:
            first_version2 = next(versions2)
        except StopIteration:
            warn("SP Empty changelog. No commits were elected to be used as entry.")
            data["versions2"] = []
        else:
            data["versions2"] = itertools.chain([first_version2], versions2)
...
def parse_cmd_line(usage, description, epilog, exname, version):
    ...
    parser.add_argument('-c', '--config',
                        help="set the gitchangelog.rc",
                        action="store", dest='path_config', default=None)
    parser.add_argument('-g', '--git',
                        help="set git path",
                        action="store", dest='path_git', default=None)
    parser.add_argument('-tag', '--tag_filter',
                        help="tag_filter",
                        action="store", dest='tag_filter', default=None)
    parser.add_argument('-ef', '--excel_path',
                        help="excel_path",
                        action="store", dest='excel_path', default=None)
    parser.add_argument('-msg', '--log_msg',
                        help="log_msg",
                        action="store", dest='log_msg', default=None)
    parser.add_argument('-sp', '--sp_dir',
                        help="sp_dir",
                        action="store", dest='sp_dir', default=None)
    parser.add_argument('-paxmodel', '--paxmodel',
                        help="paxmodel",
                        action="store", dest='paxmodel', default=None)
    parser.add_argument('-paxproduct', '--paxproduct',
                        help="paxproduct",
                        action="store", dest='paxproduct', default=None)
    parser.add_argument('-paxver', '--paxver',
                        help="paxver",
                        action="store", dest='paxver', default=None)
...
def main():
    ...
    print('debug_varname:',debug_varname)
    ...
    try:
        if opts.path_git:
            repository = GitRepos(os.path.realpath(opts.path_git));
        else:
            repository = GitRepos(".")
    ...
    # path_config = normpath(opts.path_config, cwd=repository.toplevel) if opts.path_config else None
    try:
        path_config = os.path.realpath(opts.path_config)
    except:
        path_config = ""
    ...
    if opts.tag_filter:
        config['tag_filter_regexp'] = opts.tag_filter

    # print (config['tag_filter_regexp'])
    ...
    if opts.excel_path:
        ...
        out_en = config.get("output_engine", rest_py)
    ...
    try:
        ...
        content = changelog(
            ...
            msg=opts.log_msg,
            sp_dir=opts.sp_dir,
            paxmodel=opts.paxmodel,
            paxproduct=opts.paxproduct,
            paxver=opts.paxver,
